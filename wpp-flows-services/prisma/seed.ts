...
async function seedMenu(organizationId: string): Promise<SeededMenu> {
  ...
  console.log(
    `  · 3 categorias + ${created.length} itens (com disponibilidade por dia)`,
  );
  return {
    categories: {
      pizzas: pizzas.id,
...
async function seedFlow(organizationId: string, botId: string) {
  ...
  console.log(
    `  · fluxo "${flow.name}" v${flow.version} (ativo) com ${flow.steps.length} passos`,
  );
  return flow;
}

async function seedPromotions(organizationId: string, menu: SeededMenu) {
  ...
  console.log(
    "  · 3 promoções (1 nth-order + 2 daily, sendo 1 com item destacado)",
  );
}
...
async function seedCoupons(organizationId: string) {
  ...
  console.log(
    "  · 4 cupons (1 sempre válido, 1 com janela, 1 fixo, 1 expirado/inativo)",
  );
}
...
async function main() {
  ...
  console.log(
    `  · taxa de entrega: R$ ${org.deliveryFee} · timeout: ${org.paymentTimeoutMinutes} min`,
  );
  console.log(
    `  · horário: ${org.workingStartTime}–${org.workingEndTime}, dias ${org.workingDaysOfWeek.join(",")}`,
  );
